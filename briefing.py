...
def synthesize_narratives(macro_news: str, prices: dict, stock_news: dict) -> str:
    ...
    system_prompt = (
        ...
        "SECTIONS (use exact HTML h3 tags):\n"
        "<h3>MACRO OVERNIGHT</h3> — 3 sentences max. Synthesise the macro headlines. "
        "Connect to watchlist where relevant: oil → tankers (ECO/TRMD/FRO/TEN); "
        "rates → banks (ALPHA); EUR/SEK → servicers (DOV/INTRUM). Flag moves >1% with ⚠️.\n\n"
        "<h3>ALERTS</h3> — Stock moves >2% or material news only. Omit if none.\n\n"
        "<h3>PRIVATE EQUITY</h3> — TPG: 3-4 sentences (price → volume → news → implication).\n\n"
        "<h3>GREEK BANKS</h3> — Alpha Bank: 3-4 sentences.\n\n"
        "<h3>TANKER SHIPPING</h3> — ECO, TRMD, FRO, TEN: 3 sentences each.\n\n"
        "<h3>CREDIT / NPL SERVICERS</h3> — doValue, Intrum: 3-4 sentences each.\n\n"
        "No table. No preamble. Start directly with the first <h3> tag."
    )
